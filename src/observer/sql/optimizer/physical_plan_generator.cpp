...
//   }
    const vector<unique_ptr<Expression>>& project_exp = project_oper.expressions();
    for(const unique_ptr<Expression> &exp : project_exp)
    {
        project_operator->add_expression(exp);
    }
...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, std::unique_ptr<PhysicalOperator> &oper)
{
  ...
  oper = unique_ptr<PhysicalOperator>(new UpdatePhysicalOperator(update_oper.table(), update_oper.attr(), update_oper.values()));
