    ...
    SelectStmt *select_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  std::vector<std::pair<SysFunc, Field>> all_fields = select_stmt->query_fields();
  for (Table *table : tables) {
    std::vector<Field> fields;
    for (const std::pair<SysFunc, Field> &field : all_fields) {
      if (0 == strcmp(field.second.table_name(), table->name())) {
    ...
    UpdateStmt *update_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  unique_ptr<LogicalOperator> update_oper(new UpdateLogicalOperator(table, update_stmt->attr(), update_stmt->values()));
