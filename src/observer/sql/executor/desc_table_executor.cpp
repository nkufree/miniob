...
RC DescTableExecutor::execute(SQLStageEvent *sql_event)
{
  ...
  if (table != nullptr) {
    ...
    tuple_schema.append_cell(TupleCellSpec("", "Allow NULL", "Allow NULL"));
    ...
    for (int i = table_meta.sys_field_num(); i < table_meta.field_num(); i++) {
      ...
      oper->append({field_meta->name(), attr_type_to_string(field_meta->type()), std::to_string(field_meta->len()), field_meta->allow_null() ? std::string("true") : std::string("false")});
