...
{
    // LOG_WARN("goto compare left.is_null=%d",(int)left.is_null());
    if(comp_ == IS_NULL) 
    {
        result = left.is_null();
        return RC::SUCCESS;
  ...
  switch (comp_) {
    ...
    case OP_LIKE:
    case OP_NOT_LIKE: {
        std::string raw_reg(right.get_string());
        ...
        bool res = std::regex_match(left.get_string(), reg);
        if((res && comp_ == OP_LIKE) || (!res && comp_ == OP_NOT_LIKE))
            result = true;
        else
            result = false;
        break;
